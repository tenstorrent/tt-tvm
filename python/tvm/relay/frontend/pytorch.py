...
class PyTorchOpConverter:
    ...
    def __init__(self, prelude, default_dtype, input_structure_with_tensor,use_parser_friendly_name=False):
        self.prelude = prelude
        self.default_dtype = default_dtype
        self.input_structure_with_tensor = input_structure_with_tensor
    ...
    def zeros(self, inputs, input_types):
        ...
        input_structure_with_tensor = self.input_structure_with_tensor
        data_computed= []
        for d in data:
            if type(d) == _expr.Call:
                variable = _analysis.free_vars(d)
                value  = _infer_value(d,{variable[0].name_hint:np.array(input_structure_with_tensor[variable[0].name_hint][0])}).numpy().item()
                data_computed.append(value)
            
            if type(d) == int:
                data_computed.append(d)
        return self.full_impl(data_computed, 0, dtype)
...
def from_pytorch(
    ...
    converter = PyTorchOpConverter(prelude, default_dtype,input_structure_with_tensor,use_parser_friendly_name)
